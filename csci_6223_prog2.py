...
class TokenClass:
	...
	def _checkSymbol(self):
		if self.curr not in reserved:
			...
			elif not self.flags.isIf and not self.flags.isWhile and not self.flags.rp and self.flags.isDecl:
				self.syms.append(self.curr)
				self.flags.isDecl = False
				if not checkSymbolName(self.curr) or self.curr.isdigit() or is_float(self.curr):
	...
	def _checkAssign(self):
		...
		elif  self.curr in reserved and \
			self.curr not in ['+','-','*','/','%','(',')',';'] and "arithmetic" not in self.message:
				self.message += "\n- statement contains non arithmetic tokens"
		elif self.curr.isdigit() or is_float(self.curr) or self.curr in self.symbols:
			self.flags.numSyms += 1
		elif self.curr in ['+','-','*','/','%']:
			self.flags.numOps += 1
		elif self.curr == '(':
			if self.next == ')':
	...
	def checkCurr(self):
		...
		if not self._checkBlock():
			if "permitted" not in self.message:
				self.message += "\n- statement not permitted"
		if self.flags.inVarDecl:
			if self.curr in ['read','print']:
				self.flags.rp = True
			if self.curr == "if":
				self.flags.isIf = True
			if self.curr == "while":
				self.flags.isWhile = True
			if isType(self.curr) and self.j == 1:
				self.flags.isDecl = True
			self._varErrors()
		elif self.flags.inStmtBlk:
			self._stmtErrors()
	...
	def _printLine(self):
		...
		if self.curr in reserved:
			...
			if self.curr not in self.symbols and self.j == 1:
				if not self.flags.undefined:
					sys.stdout.write("\t undefined symbol")
				self.flags.undefined = True
			if "characters" in self.message:
				sys.stdout.write("\timproper symbol character set")
			if self.curr not in self.symbols and self.curr not in self.syms:
				if not self.flags.undefined:
					sys.stdout.write("\tundefined symbol ")
				self.flags.undefined = True
			...
			if self.flags.inStmtBlk:
				sys.stdout.write("var_code\tundefined symbol")
				return
			sys.stdout.write("undefined")
			if "undefined" not in self.message:
				self.message += "\n- undefined symbol"
	...
	def _checkAddSymbol(self):
		...
		for i in self.syms:
			if i not in reserved and i not in self.symbols and not i.isdigit() and not is_float(i):
	...
	def _stmtBlockLineCheck(self):
		...
		if self.flags.isAssign:
			...
			if (self.flags.numSyms-1 != self.flags.numOps):
				self.message += "\n- missing symbol or mathematical operation"
			if len(self.flags.parenStack) > 0 and "improper parenthesis nesting" \
					not in self.message:
				self.message += "\n- improper parenthesis nesting"
#        			s  o
...
#
	...
	def checkLine(self):
		...
		if self.flags.newline:
			...
			self.flags.isWhile = self.flags.isIf  = False
			self.flags.rp = self.flags.isAssign = self.flags.isDecl = False
...
def main():
	...
	string = open(args.FILE, "r").read().strip().lower()
